...
createCart  = async (req,res)=>{
    ...
    // if(userId!=req.decodedToken) return res.status(403).send({status:false,msg:"user is not Authenticate"})
    ...
    if(cart.userId!=userId)return res.status(404).send({status:false,msg:"cart is not valid"})
    ...
    priviousCart.totalPrice=priviousCart.totalPrice+product.price
    priviousCart.items.push({ productId: productId, quantity: priviousCart.items[priviousCart.items.length-1].quantity+1 })
